...
class LoginController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if($email!=null and $clave!=null and Jugador::where('email', '=', Input::get('email'))->exists()  ) {
            ...
            if ($checkPass) {
                $options=null;
                $URL_PERFIL=Cloudder::show("fotoPerfil".$login[0]->id_jugador, array ("width" => 250, "height" => 250) );// poner , array ($options) sino funciona
                return response()->json(['api_token' => $login[0]->api_token, 'success' => true,'img_perfil'=>$URL_PERFIL]);//,'img_perfil'=>$URL_PERFIL]
            } else {
